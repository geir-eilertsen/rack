...
**And a word the rack never uses is left out, last of all.** "wireless computer mouse" found nothing for the Logitech wireless mouse in lab/10: every word must match and no item says "computer"; the expander adds words and never takes one away, and it answered with nothing because the other two were already the rack's words. Neither pass can rescue a query that is over-specified by one word. So when neither has landed, `FindItems.smart` asks the index which words of the query occur in no item at all, searches the rest under the same all-words rule at 0.8 weight, and reports them as `ignored_words` — `find.html` says *Nothing mentions "computer", so that was left out*, because a hit that does not match every word typed needs explaining. Kept only when it lands convincingly or when there was nothing else: "isolating tape" widened to the electrical tape must not also drag in the resistors on tape reels through "tape" alone. The suggest log line says `leaving out [...]` for the same reason it says `after widening to`.
