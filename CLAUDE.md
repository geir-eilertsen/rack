...
### What a thing goes with

A phone in one box and its charger in another are both filed correctly and both findable, and neither entry says anything about the other — so the rack is right about both and still sends you to two rooms for one thing (#31). `FindCompanions` is the counterpart lookup: `QueryExpander.goesWith` asks the same small model the same grounded question in the other direction — not *what else is this called* but *what is this used with*, in the words this rack uses — and each answer is searched like an expanded term. Hits in the item's own slot are reported apart, as `together` rather than as a move, because the question is what to bring together and those already are — the Lumix camera sits beside its charger, and a panel saying it found nothing anywhere else would be true and would hide the pair it found.

**It is a suggestion, never a move.** A move is a physical act the app cannot do, and the record must not lead the drawer. The offer is the counterpart's drawer with the room under it and a move beside it, through the existing `MoveItem`; which half moves is the user's call, because only they know which box is the phone's home. Nothing is stored — a "goes with" link would be a second place a fact lives and would drift the moment one half was used up or moved, so the pair is rediscovered from the names each time and cached per name.

**Two places, both already on screen.** Filing is where a pair gets split — a charger filed by likeness goes in with the other chargers and the phone stays where it was — so `SuggestSlot` runs the lookup on every extracted item and `put.html` offers the counterpart's drawer among the suggestions, dashed and reading *goes with: Phone*, or adds that line to a drawer likeness already found. And an expanded item row — on a search hit in `find.html`, and on an item in a drawer in `put.html` — has *What does this go with?* between the merge and move panels: `GET /c/{c}/{s}/items/{i}/companions`, fetched on open rather than for every row on every draw, with *Move this there* and *Bring it here* on each row and *Already here* for a counterpart in the same slot. `assets/companions.js` renders and wires it for both pages, the way `itemdocs.js` does, with the page supplying what only it knows: how to label a drawer, where a container is, and what to redraw after a move. The whole-index audit the issue also describes is not built.

**A second charger is not a counterpart of a charger.** The prompt says so, and `SpringAiQueryExpander.clean` drops a term built only from the item's own words, but "phone" searched literally for a USB charger still matches the phone charger in the next drawer. `FindCompanions.sameKind` reads a hit's name for what kind of thing it names — a compound is named by its last word, so a phone charger is a charger and an old phone is a phone — and a hit naming the same kind as the item is another of the same thing. Word overlap will not do: "Phone charger" shares a word with "Phone" and is exactly the answer.

**And a counterpart is a thing, found by its name.** Probed against this rack on the day it shipped, the model answered "automotive" for a CAN-bus interface and "5V" for a USB wall outlet — a tag and a property, not things — and "automotive" matched the paper towels on their tag while "5V" is a substring of every 35V capacitor's name. So a term with no run of three letters in it is dropped (`namesAThing`), and a hit is kept only when it scored at least 3, the weight of a name or part-number match: a tag in common is a category in common, not a pair.
