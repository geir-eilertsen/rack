...
**Two places, both already on screen.** Filing is where a pair gets split — a charger filed by likeness goes in with the other chargers and the phone stays where it was — so `SuggestSlot` runs the lookup on every extracted item and `put.html` offers the counterpart's drawer among the suggestions, dashed and reading *goes with: Phone*, or adds that line to a drawer likeness already found. And a search hit has *What does this go with?* under the merge and move panels: `GET /c/{c}/{s}/items/{i}/companions`, fetched on open rather than for every hit on every keystroke, with *Move this there* and *Bring it here* on each row. The whole-index audit the issue also describes is not built.
