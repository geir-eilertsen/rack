...
**And the camera frame is never decoded at the size the camera took it.** That was the other half of the memory: a 12-megapixel frame decodes to 48MB and a 50-megapixel one to 200MB, held in full until the resize was done — right as the phone wanted the memory back for the next shot. `createImageBitmap` will decode a JPEG at a fraction of its size and never hold the whole frame, but only if told the size it wants before it starts, and it cannot be asked a picture's size without decoding it. So `rackPhotos.resize` reads the width and height off the JPEG's own frame header (`jpegSize`, a few hundred bytes of marker-walking) and asks for 1568 up front. One dimension is given and the browser keeps the ratio: which edge is the width depends on an EXIF orientation tag the parser does not read, and a wrong guess at the pair would squash the picture where a wrong guess at one edge only leaves it a little larger than asked, for the canvas to finish. A file that is not a JPEG decodes as it comes, as before. Checked against ImageIO on all 311 photographs in this rack.

**The pending strip shows previews, not the photographs.** An `<img>` is decoded at the size it was given rather than the size it is drawn at, so six 88px thumbnails backed by the resized photographs were six 1568px pictures in memory, seven megabytes each, for exactly as long as the camera was open for the seventh. `rackPhotos.preview` makes a 264px copy for the strip; the photograph itself stays a JPEG in a `File` until it is filed.
