...
**Nothing it cites is believed until the index agrees.** A reference has to resolve to an item the rack holds, under the name it was cited with; an invented one is dropped, so is a citation for a subject that was not asked about, and so is the subject citing itself. Same rule as the checklist's `verify`. The answer is cached per subject *by name*, and the drawer is looked up on every request, so a pair whose halves have since moved still resolves to where they are now. What is already in the subject's own slot comes back as `together` rather than as a move — the Lumix camera sits beside its charger, and a panel saying it belongs with nothing elsewhere would be true and would hide the pair it found.
