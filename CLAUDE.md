...
**And a container with several is drawn at the width its slot ids imply.** A `Container` keeps a flat list of slots — the cols and rows given at registration are spent producing that list and not kept — so `.slots-grid` was five across for everything: right for the 5×12 rack it was written for, wrong for the 2×5 cupboard, which came out as `A1 B1 A2 B2 A3 / B3 A4 B4 A5 B5`, ten drawers in the correct order and the wrong shape. Nothing new is stored, because nothing needs to be: `ContainerLayout.grid` emits row-major with one letter per column, so a list whose rows are all the same width *is* that wide, and `assets/slots.js` reads it back off the ids. Anything with no shape to read falls back to five — a linear `1..11`, a free-form `top-left` — and so does a one-wide reading, since `linear(4, "Box")` gives `Box1..Box4`, a numbered run rather than a column of four. Derived in the page rather than served, the way a slot's frames are.

**And a cabinet of mixed drawer sizes is bands, one after another** (#30). A band of six small drawers over a pair of large ones is neither a grid nor a numbered run: it is both, so `layout.kind: "sections"` takes a list of grid or linear blocks and `ContainerLayout.sections` concatenates them into the one flat list a container has always held. Nothing downstream notices — items, search, moves, resync and the QR encoding are per slot and have no opinion about arrangement.

**Bands letter their columns from A alike and carry the row numbering on**, so those two large drawers are `A5 B5` under `A1..F4`. That is what keeps the ids unique without anybody being asked to choose letters for a band, and — the part that is not merely convenient — it is what lets a band *one row deep* be told apart from the row above it. Restarting the numbering would give `A1..F1` then `A1 B1`, either a collision or, lettered on past F, `G1 H1`: eight ids sharing a row number, which reads as one row of eight and is a picture of a cabinet nobody owns. A band may still name its own columns (`letters: "LM"`), which is cosmetic — the numbering runs on regardless.

**A ragged reading is now the shape rather than a failure.** `slots.js` used to treat rows of differing widths as *no shape* and fall back to five; for this cabinet ragged **is** the shape, so each row is drawn at the width its own ids imply. One grid still, laid on however many tracks every row divides into (six, for rows of six and two), with each slot spanning its share and each row's first slot starting a fresh line — so a row that does not divide the tracks evenly still gets a line to itself. Past 24 tracks the widest row sets the width and a narrower one leaves a remainder: rows of five and seven want 35 tracks, and 34 gaps on a phone is mostly gap. `rackSlots.render` owns filling the grid and shaping it, in that order, because the shaping reads the elements it is shaping.

**A container is still one label scale**, so a mixed cabinet's small drawers set the size of the large ones' stickers. Left as it stands rather than given a per-band override: QR module size is the real floor (~0.46 for a readable symbol), a scale that suits the small drawers is legible on the large ones, and a per-slot scale is a second place for a container's shape to be recorded.
...
- Containers are defined in `application.yml` under `rack.containers` and materialised at boot by `ContainerRegistry` (see `application/RackConfiguration.java`). Layout kinds: `grid` (cols × rows → A1..), `linear` (N → 1..N with optional prefix), and `sections` (a list of either, concatenated — see above).
...
1. **Keyword** (`JsonFilePartIndex.searchByKeyword`) — the query is split on whitespace and scored word by word, so "black electrical tape" finds "Electrical tape / one black roll" even though no field contains that phrase. Per term: part number or `name` +3, `description` +2, category or a tag +1, plus +3 if the words appear in the order typed. Splitting on whitespace only keeps `TO-220` one term.
   ...
   **And a wrong answer can be forgotten.** `DELETE /c/{c}/{s}/items/{i}/qa/{n}` (`ForgetAnswer`) drops one exchange; the `×` on each entry in both list pages is the way in. A stored answer is read as fact every time the row is opened, and it is searched, so a confidently wrong one is worse than none — it can put an item under a query it has no business matching. Only the person who asked can tell it was wrong. Forgetting one does not stamp `lastVerified`: deleting a sentence at a desk is not looking in the drawer.
...
**And a word too many is left out, last of all.** "wireless computer mouse" found nothing for the Logitech wireless mouse in lab/10: every word must match and the mouse does not say "computer" — the Raspberry Pi and the case screws do, so the word is not even absent from the rack; the expander adds words and never takes one away, and it answered with nothing because the other two were already the rack's words. Neither pass can rescue a query over-specified by one word. So when neither has landed, `FindItems.smart` leaves each word out in turn and searches the rest under the same all-words rule at 0.8 weight. A rest that lands convincingly is kept and the word it did without comes back as `ignored_words` — `find.html` says *Also searched without "computer"*, because a hit that does not match every word typed needs explaining. A rest that does not land is shown only when there was nothing else at all: "isolating tape" widened to the electrical tape must not also drag in the resistors on tape reels through "tape" alone, but with the expander down "tape" alone, labelled, beats an empty page. The suggest log line says `leaving out [...]` for the same reason it says `after widening to`.
...
- `/discuss.html`, `POST /discuss` → talk a build through against the shelf: the whole conversation goes with each message, every suggestion cites a drawer and is verified against the index (see *Talking a build through*)
...
### Talking a build through

`POST /discuss` (`DiscussBuild`) and `/discuss.html`, reached from the hub as **Build something**. `/ask` answers one question with a checklist: does the rack hold what a *known* job needs. This is the conversation before the job is known — "I need a 15 V DC supply for a small amp", and the answer is the laptop brick on the lab shelf with a buck module from the rack, or the transformer inside the dead amplifier in the garage box, and which of those is the sensible one. You can ask why, push back, and change the plan; the whole exchange goes back with every message, so the model builds on what was said rather than starting again.

**Same two rules as the checklist.** The whole index goes in — the point of asking what could be cannibalised is that the answer may be a thing nobody would search for, which is exactly what a retrieval step would drop. And every suggestion names a drawer and an item, and `DiscussBuild.verify` puts it through `AskAboutRack.keepReal`: repaired where only the punctuation is wrong, dropped where that drawer does not hold that item. The model is not permitted to furnish the rack from memory here either.

**Each suggestion says what kind of suggestion it is** — `use`, `adapt`, `substitute` or `cannibalise` — because taking something apart is a decision to make with open eyes, and the prompt asks for the straightforward use before the adaptation before the teardown. The page badges them (*use as is*, *adapt*, *near enough*, *take apart*). Anything else the model writes for a kind is read as a plain use. `missing` is what they would still have to get, one entry each.

**The model's own turns are replayed as the JSON they were**, not as the prose they were drawn as. A history of plain paragraphs invites a plain paragraph back, and a paragraph is not a reply this can read. The browser keeps each assistant turn with its suggestions for that reason. Only the last 40 messages go up when a conversation runs long — the listing is the bulk of every call, an hour's chat is small beside it, but not unbounded — and a tail cut so it opens with the model's turn is trimmed to open with the user's.

**Nothing is stored on the server.** A discussion is a proposal under review, like a plan, and lives in `localStorage` until something comes of it, with *Start over* to drop it; a refresh, or following a drawer link and coming back, does not cost the exchange again. The page says when the conversation started, because the drawers may have moved since. Same trade the pending photo strip makes: it does not follow you to another device. Runs on `rack.ai.ask-model` (Sonnet) with `discuss-max-tokens` for the reply; the listing dominates the cost, a few cents a turn.
...
- **Everything about a container except its id and its shape is editable.** Name, the word for one of its places, label scale, where it physically is, and free-form notes. Reshaping a container would orphan slots that hold items, so `UpdateContainer` never rewrites the slot list.
- **`location` is the only thing in the app that says which room to walk to.** The index answers down to a drawer and then stops: `lab/10` is precise and useless to somebody who does not already know where `lab` is. Free text rather than a room picked from a list — a list of rooms is one more thing that has to be kept true, and the half of the answer doing the work ("north wall") is the half no list has a field for. `notes` is the same field with more room, for what a container is rather than where: which of two identical boxes this is, what its slot ids mean.
- **Absent means unchanged; blank means cleared.** A name has no third state, so `PATCH` still refuses a blank one — but a location you turn out to be wrong about is better empty than wrong, so emptying the box is how you say so. Both are trimmed in `Container`'s constructor and length-checked in `validLocation`/`validNotes`, the same split `validLabelScale` uses: `containers.json` is one file, so a throw at construction is every container at once rather than one bad field.
- Registration takes a location, because standing in front of a box you have just bought is when you know where it is going. It does not take notes — a note is something that accrues — though `POST /c` accepts both, so a container made over HTTP need not be PATCHed to be complete.
- **A location already in use is picked, not retyped.** Both location boxes on containers.html are backed by a `<datalist>` built from the locations the other containers already carry, so the second container in the garage goes in "Garage" rather than in "garage " — one letter of difference and the shelf is in two places. Typing something new is still how a location comes into existence, so this stays a list that builds itself rather than one somebody maintains. It is refreshed from the page's own fetch rather than from `rackPlaces`' cache, and a location saved on one card is appended so the next card can offer it without a refetch.

**And a location is shown wherever the app says where something is.** A search hit's group header carries the room under the drawer, and so does every citation on a checklist and every tool a plan's step names — `lab/10` is exact and tells somebody who does not already know where `lab` is precisely nothing. `assets/places.js` answers the question in one file for `find.html` and `assets/advice.js`: which container this is, whether it is divided, what to call the place, and where it is. One `/c` per page however many callers ask, and a failed fetch answers "no containers" rather than throwing, so a name falls back to its id exactly as it did before. find.html renders the line hidden when `/c` has not landed yet and fills it in with the names, the same relabel-in-place that stops a re-render swallowing a take in flight. `put.html` keeps its own copy of the lookup: its container fetch is part of a startup sequence that reports its own failure on screen.
...
Photograph a charger and the rack should say which device it charges and which drawer that is in; photograph the device and it should say where the charger is (#31). A phone in one box and its charger in another are both filed correctly and both findable, and neither entry says anything about the other — so the rack is right about both and still sends you to two rooms for one thing. `FindCompanions` answers it, in three places: among the filing suggestions on `put.html` (`SuggestSlot` runs it on every extracted item, so the card reads *belongs with: Raspberry Pi 4 (powers it over USB-C)* with the room under it), and on an expanded item row on both `put.html` and `find.html` as *What does this go with?*, fetched on open through `GET /c/{c}/{s}/items/{i}/companions`. `assets/companions.js` renders and wires that panel for both pages, the way `itemdocs.js` does.

**The model is shown the rack, not asked to guess words.** The first version bridged the item's name to the rack's vocabulary through the expander and keyword-searched the result, and every live try broke a different way: "automotive" matched the paper towels on a tag, "5V" every 35V capacitor by substring, and "Raspberry Pi" — the one term that would have found the Pi for its PSU — was dropped for adding no new word, a rule that is right for synonyms and wrong for the other half of a pair. Each break got a heuristic and the next break went round it. Reading "5.1V 3A USB-C power supply" beside "Raspberry Pi 4" is a matching job, and the whole listing fits in one prompt — the same argument the project checklist makes — so `PairFinder` is handed the subjects and every other entry as `container/slot#index | name | description | part number | category`, and cites the references each belongs with and why.

**It gives a verdict on every candidate, and only "pair" is kept.** Asked for citations alone, Sonnet cited the ceiling rose beside a USB wall outlet and wrote *merely related, not a pair* in the reason — it knew, and the format gave it no way to say so. So each candidate carries `pair`, `same kind` or `related`, and `SpringAiPairFinder.pairs` keeps the first. That is the model's judgement doing the filtering, where the first version had a head-word rule, a score threshold and a units filter doing it badly.

**And every pair it proposes is put to it again on its own.** Picking candidates out of a listing of a hundred and fifty lines and judging two entries side by side are different questions, and the second is the one it answers well: asked over the whole listing it paired the ELKO USB wall outlet with "small coil spring for Elco switch" on the brand, inventing a switch inside the outlet. `PairFinder.confirm` shows it each proposed pair alone — the two lines and its own reason — and only what it says still stands is kept and cached. One small extra call per lookup; when that call fails every claim stands, because the first opinion was the model's too. The same discipline the index check applies, aimed at the judgement instead of the reference.

**Sonnet, measured (`rack.ai.companions-model`).** On this rack's own drawers, with the verdict format, haiku cited the Raspberry Pi PSU as the pair for a Sony phone charger, a USB wall outlet and a Lumix camera charger alike. Sonnet answered nothing for those three, the camera for its charger, the Pi for its PSU, and found the RaZberry Z-Wave HAT in lab/5 for the Pi in lab/11 — a split pair the feature exists to catch, that nobody had asked about. Same call as for extraction: confidently wrong is the failure this app cannot have. About five cents a call.

**One call per filed batch, not per item.** The listing is most of the prompt, so eight items in one shot as eight calls would be eight listings and most of a minute in a row. `executeAll` numbers the extracted items `S1..Sn` as subjects of one prompt and the model answers per subject; items already answered are served from the cache and left out of the call.

**Nothing it cites is believed until the index agrees.** A reference has to resolve to an item the rack holds, under the name it was cited with; an invented one is dropped, so is a citation for a subject that was not asked about, and so is the subject citing itself. Same rule as the checklist's `verify`. The answer is cached per subject *and per listing* — the key carries a fingerprint of the lines the model was shown, because a charger asked about before the phones went into the glass cabinet was answered "nothing" and that answer outlived the cabinet's contents — and a citation is remembered by name rather than by drawer, so a moved item resolves to where it is now. What is already in the subject's own slot comes back as `together` rather than as a move — the Lumix camera sits beside its charger, and a panel saying it belongs with nothing elsewhere would be true and would hide the pair it found.

**Made for one thing, or good for many: both are real.** A Sony CST-13 charger belongs with the Sony phone and nothing else; a general-purpose USB-C charger belongs with every USB-C device on the shelf, and the prompt asks for each of them with what it does for that one. The cap is 12 rather than 5 for that reason.
...
**What the camera cannot read is typed beside the photos.** A photograph does not show a thread pitch, a length, a wire gauge or a resistor's value, and the model guesses them badly and confidently — an M4×16 and an M4×20 look the same from above. The person holding the screw has just measured it, so `put.html` has a one-line note box under the pending strip (*Anything the camera can't read?*) and sends it as a `note` field with the batch, to `/suggest`, the filing `POST` and a resync preview alike. `PartExtractor.extract(images, note)` hands it to the model as a fact that outranks the picture, to be written into the name and description of the item it describes. `CachingPartExtractor` keys on the note as well as the bytes, so a suggestion and the file after it read the same batch and a changed note is a fresh reading.

**Asking is a guess about the next reply; `FilingNote.ensure` is the check.** A batch that read as a single item can only be about that item, so if it came back without the note in its name, description or part number, the note is appended to the description. Compared with case, spacing and punctuation removed and `×` read as `x`, because the model rewrites "M4 x 20 mm" as "M4×20mm" and a guard that missed that would print it twice. Several items are left to the model's placement — which of them a note describes is exactly the judgement it was shown the photos to make. In a resync a matched item keeps its own wording, so the note only lands on what the resync adds.
